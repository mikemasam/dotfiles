...
if not snip_status_ok then 
  print("cmp not luasnip");
  return 
end

local tabnine_status_ok, _ = pcall(require, "installs.tabnine")
if not tabnine_status_ok then 
  print("cmp.tabnine not found");
  return 
end
...
local icons = require "installs.icons"
...
cmp.setup {
  ...
  sources = {
    {name = "crates", group_index = 1}, {
      ...
      group_index = 2,
      filter = function(entry, ctx)
        if not contains(buffer_fts, ctx.prev_context.filetype) then return true end
      end
    }, {name = "cmp_tabnine", group_index = 2}, {name = "path", group_index = 2},
  ...
  sorting = {
    ...
    comparators = {
      ...
      compare.score, compare.recently_used, compare.locality, -- compare.kind,
