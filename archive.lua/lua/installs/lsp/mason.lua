...
local servers = {
  ...
  --"ruby_ls", "gopls", "prismals", "volar", "solargraph", "elixirls", erlangls"
...
for _, server in pairs(servers) do
  opts = {
    on_attach = require("installs.lsp.handlers").on_attach,
    capabilities = require("installs.lsp.handlers").capabilities
  }

  server = vim.split(server, "@")[1]

  if server == "jsonls" then
    local jsonls_opts = require "installs.lsp.settings.jsonls"
    opts = vim.tbl_deep_extend("force", jsonls_opts, opts)
  end

  if server == "yamlls" then
    local yamlls_opts = require "installs.lsp.settings.yamlls"
    opts = vim.tbl_deep_extend("force", yamlls_opts, opts)
  end

  if server == "clangd" then
    local clangd_opts = require "installs.lsp.settings.clangd"
  ...
  if server == "sumneko_lua" then
    ...
    -- local sumneko_opts = require "installs.lsp.settings.sumneko_lua"
  ...
  if server == "tsserver" then
    local tsserver_opts = require "installs.lsp.settings.tsserver"
    vim.tbl_deep_extend("force", tsserver_opts, opts)
  end

  if server == "pyright" then
    local pyright_opts = require "installs.lsp.settings.pyright"
    vim.tbl_deep_extend("force", pyright_opts, opts)
  end

--  if server == "gopls" then
--    local gopls_opts = require "installs.lsp.settings.gopls"
--    vim.tbl_deep_extend("force", gopls_opts, opts)
--  end
--
--  if server == "solc" then
--    local solc_opts = require "installs.lsp.settings.solc"
--    vim.tbl_deep_extend("force", solc_opts, opts)
--  end
--
--  if server == "emmet_ls" then
--    local emmet_ls_opts = require "installs.lsp.settings.emmet_ls"
--    opts = vim.tbl_deep_extend("force", emmet_ls_opts, opts)
--  end
--
--  if server == "zk" then
--    local zk_opts = require "installs.lsp.settings.zk"
...
--  end
  ...
  if server == "rust_analyzer" then
    local rust_opts = require "installs.lsp.settings.rust"
